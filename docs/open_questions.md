...
**採番のきまり**: 各項目は `OQ-001` 形式の番号を持つ。
**この番号は採番後に変えない。決まっても欠番にせず、決定の記録として残す。**
他のドキュメントやコードからは、見出しの位置（「3番」など）ではなく **ID で参照する**。
以前は位置で参照していたため、項目が1つ決まるたびに参照先がずれ、
`poc_evaluation.py` が存在しない「7番」を指すといった食い違いが実際に起きていた。
